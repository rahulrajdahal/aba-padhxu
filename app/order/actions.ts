  ...
  try {
    const body = {
      ...
      amount: formData.get("amount")
        ? Number(formData.get("amount") as string)
        : 1000,
    ...
    cartItems.forEach(
      async (cartItem: {
        book: { id: string; quantity: number; name: string; sellerId: string };
        quantity: number;
      }) => {
        ...
        formData.append(
          "description",
          `Your item ${cartItem.book.name} has been placed for order by ${user.name}`,
        );
...
export const sendOrderEmail = async (user: Pick<User, "email">) => {
  try {
